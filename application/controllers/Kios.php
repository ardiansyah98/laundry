...
class kios extends CI_Controller {
	...
	function pembayaran_transaksi($index){
		...
		if($jenis[0]=="single"){
			...
			if($transaksi->id_cabang==$cabang->id_cabang){
				$update["status_pembayaran"]="Sudah";
				$update["tgl_bayar"]=$tanggal;
			...
			foreach ($transaksi as $row) {
				if($row->id_cabang==$cabang->id_cabang){
	...
	function ambil_transaksi($index){
		...
		if($jenis[0]=="single"){
			...
			$paket=$this->DbCore->get_data_1param("paket","id_paket",$jenis[1])->row();
			if($transaksi->id_cabang==$cabang->id_cabang){
				$update["status_cucian"]="Diambil";
				$update["tgl_diambil"]=$tanggal;
				$this->DbCore->update_data("transaksi","id_transaksi",$jenis[1],$update);
				$this->set_log_transaksi("status_cucian","Sudah",$jenis[1]);
			}
		}elseif($jenis[0]=="all"){
			$paket=$this->DbCore->get_data_1param("paket","kode_paket",$jenis[1])->row();
			$transaksi=$this->DbCore->get_data_1param("transaksi","id_paket",$paket->id_paket)->result();
			foreach ($transaksi as $row) {
				if($row->id_cabang==$cabang->id_cabang){
					if($row->status_pembayaran=="Belum"){
						...
						$update["tgl_diambil"]=$tanggal;
						$this->DbCore->update_data("transaksi","id_transaksi",$row->id_transaksi,$update);
						$this->set_log_transaksi("status_cucian","Sudah",$row->id_transaksi);
					}
				}
			}
		}
		$update_paket_pembayaran["status_pengambilan_paket"]="Sudah";
		$this->update_paket_by_chcek_transaksi($paket->id_paket,"status_cucian","Diambil",$update_paket_pembayaran);
		redirect("kios/transaksi/$paket->id_paket");
	}
	...
	function ajax_list(){
		...
		// $list=$this->DbCore->get_data_1param("transaksi","id_cabang",$cabang->id_cabang)->result();
		$data = array();
		$no = $_POST['start'];
